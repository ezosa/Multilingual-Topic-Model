...
stopwords_denews = set(stopwords.words('english') + ['said', 'would', 'mr', 'according']).union(stopwords.words('german') + ['sagt', 'sagte', 'fuer', 'sei', 'dass', 'ueber', 'worden', 'wurden', 'heute'])
...
def GetNewsCorpus(path, languages):
	...
	for f in filenames:
		...
		for i in range(len(index_start) - 1):
			...
			article = clean_denews(text[start_art:end_art]).split()
...
def get_wikipedia_corpus(num_docs=1000):
	wiki_fi_sv_path = "../data/wiki/wiki_align_fi-sv.pkl"
	wiki = pickle.load(open(wiki_fi_sv_path,'rb'))
	languages = ['fi', 'sv']
	documents = {lang: [] for lang in languages}
	dictionary = {lang: set() for lang in languages}
	keys = list(wiki.keys())
	random.shuffle(keys)
	selected_keys = keys[:num_docs]
	for k in selected_keys:
		print("Processing Wikipedia article",k)
		wiki_align = wiki[k]
		if len(wiki_align) == 2:
			for lang in languages:
				wiki_art = wiki_align[lang].split()
				wiki_art_clean = clean_yle(wiki_art).split()
				documents[lang].append(wiki_art_clean)
				dictionary[lang].update(wiki_art_clean)
	print("Documents per language:")
	for lang in languages:
		print(lang.upper(), " - ", len(documents[lang]), "documents")
		dictionary[lang] = list(dictionary[lang])
	return documents, dictionary
